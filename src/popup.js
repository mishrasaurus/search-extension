    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      if (!currentTab) {
        ...
        document.getElementById("results").innerHTML =
          '<p class="no-results">Error: Unable to access current page. Please try again.</p>';
        return;
      }

      chrome.tabs.sendMessage(
        currentTab.id,
        { action: "getResults", websites: websites },
        (response) => {
          if (chrome.runtime.lastError) {
            console.error(chrome.runtime.lastError);
            document.getElementById("results").innerHTML =
              '<p class="no-results">This extension only works on Google, Bing, and DuckDuckGo search result pages.</p>';
            return;
          }
